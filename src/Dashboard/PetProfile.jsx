...
// Helper function to find pet by ID (make sure ID types match)
function findPetById(pets, id) {
  return pets.find((pet) => pet.id === id);
}

// Dummy isOwner function — replace with your actual logic if needed
function isOwner(pet, user) {
  // Example: check if pet.ownerId === user.id or similar logic
  return true; // allow edit for now
}
...
export default function PetProfile({ pets = [], setPets, user }) {
  ...
  useEffect(() => {
    const petId = Number(id); // Convert id from string to number
    const foundPet = findPetById(pets, petId);
    if (!foundPet) {
  ...
  const submitEdit = () => {
    const updatedPet = {
      ...editData,
      age: Number(editData.age), // Make sure age is a number
    };
